...
func NewDefaultRecommendTask(logger *zap.Logger) idl.RecommendTask {
	return newDefaultRecommendTask(logger)
}

func newDefaultRecommendTask(logger *zap.Logger) *defaultRecommendTask {
	return &defaultRecommendTask{
		logger: logger,
	}
}

func (rt *defaultRecommendTask) InitTask(ctx context.Context) {
	if err := rt.setRecommendSliceCache(ctx); err != nil {
		panic(err)
	}
...
func (rt *defaultRecommendTask) setRecommendSliceCache(ctx context.Context) error {
	// 推荐切片写入 csv 文件
	if err := rt.saveRecommend2CsvFile(); err != nil {
		return err
	}

	// 从 csv 文件获取切片排好序并缓存至本地
	rt.cacheRecommend2Local(ctx)
...
func (rt *defaultRecommendTask) startTick() {
	...
	go func(ticker *time.Ticker) {
		...
		for {
			select {
			case <-ticker.C:
				if err := rt.setRecommendSliceCache(context.Background()); err != nil {
					rt.logger.Error("task.setRecommendSliceCache error", zap.Error(err))
				}
...
// 从 csv 文件获取切片排好序并缓存至本地
func (rt *defaultRecommendTask) cacheRecommend2Local(ctx context.Context) {
	// 从 csv 文件获取切片
	...
	recommendRepo.SetCache(recommendSlice)
}
